						...
						<div className="uk-height-large uk-background-cover uk-light uk-background-cover uk-background-fixed" style={{ backgroundImage: 'url(img/gabir_bg.jpg)', minHeight: '100vh', height: 'auto' }}>
							<div className="uk-width-1-2@m uk-text-center uk-margin-auto uk-margin-auto-vertical uk-container uk-position-center">
								<div className="uk-animation-slide-top-medium uk-margin uk-width-large uk-margin-auto uk-card uk-card-secondary uk-card-body uk-box-shadow-large" style={{ maxHeight: '95vh' }}>
									...
									<p className = "tools-number" id="color-value"><span className = "uk-text-bold">Current Color: </span>{color}</p>
									<p className = "tools-number" id="metal-value"><span className = "uk-text-bold">Metallic: </span>{Math.ceil(metallic * 2.55).toString(16).toUpperCase()}</p>
									<p className = "tools-number" id="rough-value"><span className = "uk-text-bold">Roughness: </span>{Math.ceil(roughness * 2.55).toString(16).toUpperCase()}</p>

									<p className="uk-text-warning uk-text-bold">Warning: Some images may take a while to load</p>
								</div>
							</div>
						</div>
					</>
